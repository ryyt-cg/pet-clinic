...
func Test_visitById_BadRequest(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "/v1/visits/invalid-id", nil)
	resp, _ := r.Test(req, fiber.TestConfig{})
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			case http.StatusNotFound | http.StatusInternalServerError:
				...
				assert.Equal(t, tc.expectedResponses.(*resterr.ErrorResponse).Status, actualVisitResponse.Status)
				assert.Equal(t, tc.expectedResponses.(*resterr.ErrorResponse).Message, actualVisitResponse.Message)
			}
		})
	}
}
