...
func (router *Router) getAll(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			log.Error().Err(err).Msg("find no pets.")
			c.JSON(http.StatusNotFound, resterr.NotFound("No pets found."))
			return
		}
...
func (router *Router) getByName(c *gin.Context, param api.NameParam) {
	...
	if err != nil {
		...
		log.Error().Err(err).Str("name", param.Name).Msg("Unable to get pets by name.")
		// If there is an error other than not found, return internal server
